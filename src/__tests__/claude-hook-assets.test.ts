...
const GROK_BUILD_OUTGOING_ASSET_SHA256 = "6fe1ef55363e6e3f38253ca94b17ec50c942352c00d8431cd2ff2f6858c094b6";
...
describe("separate deterministic shell corpora", () => {
	...
	it.each([
		["plain body", "python3 - <<'PY'\nvalue = '''GETs'''\nPY"],
		["apostrophe body", "python3 - <<'PY'\nvalue = '''GET's'''\nPY"],
		["dangerous-looking body", "python3 - <<'PY'\nvalue = '''rm -rf /\\ncat ~/.ssh/id'''\nPY"],
		["absolute python", "/opt/venv/bin/python3 <<'PY'\npass\nPY"],
		["stdin dash", "python3 - <<\"PY\"\npass\nPY"],
		["stdout redirection before", "python3 > out <<'PY'\npass\nPY"],
		["stdout redirection after", "python3 <<'PY' >> out\npass\nPY"],
	])("rejects bounded quoted Python heredoc as unsupported: %s", (_name, command) => expect(runtime.evaluateEvent(event("Bash", { command }))).toEqual({ allowed: false, ruleId: "shell.unsupported-interpreter" }));
	it.each([
		["wrapped python", "env python3 <<'PY'\npass\nPY"],
		["unquoted delimiter", "python3 <<PY\npass\nPY"],
		["tab-stripping heredoc", "python3 <<-'PY'\npass\nPY"],
		["multiple heredocs", "python3 <<'PY' <<'NEXT'\npass\nPY\nNEXT"],
		["wrong terminator", "python3 <<'PY'\npass\nNOT_PY"],
	])("fails closed for unsupported Python heredoc grammar: %s", (_name, command) => expect(runtime.evaluateEvent(event("Bash", { command }))).toEqual({ allowed: false, ruleId: "shell.obfuscated-interpreter" }));
	it.each([
		["destructive before", "rm -rf /; python3 <<'PY'\npass\nPY", "shell.destructive-root"],
		["destructive after header", "python3 <<'PY'; rm -rf /\npass\nPY", "shell.destructive-root"],
		["destructive after terminator", "python3 <<'PY'\npass\nPY\nrm -rf /", "shell.destructive-root"],
		["force push before", "git push --force origin main; python3 <<'PY'\npass\nPY", "shell.force-push"],
		["sensitive substitution after", "python3 <<'PY'\npass\nPY\necho $(cat ~/.ssh/id)", "shell.sensitive-read"],
		["encoded payload after", "python3 <<'PY'\npass\nPY\nbase64 -d payload | bash", "shell.pipe-to-shell"],
		["protected config after", "python3 <<'PY'\npass\nPY\nprintf x > .claude/settings.json", "shell.managed-config-tamper"],
	])("preserves outer shell denial around quoted Python heredoc: %s", (_name, command, ruleId) => expect(runtime.evaluateEvent(event("Bash", { command }))).toEqual({ allowed: false, ruleId }));
