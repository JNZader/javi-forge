...
## Fix Round F4 — R3-F3-N1 (relative-root invocation gap)

User-authorized fix (F4, the fourth override of the lockout class family). Single-line fix applied inline by the orchestrator: the coverage walk now starts from the resolved ABSOLUTE root (`rootAbs`) instead of the caller's raw `dir`. With a relative invocation (`javi-forge plugin import <relative-dir>`), `walkDirs` previously held caller-relative paths while `declaredAbs` is always absolute → the two-tier real-dir lookup never matched → every declared dir fell back to its manifest path and an EXISTING declared file reported `unscannable` (force would lift it and install un-scanned content). Starting from `rootAbs` also keeps `undeclared`/`symlinks`/`errors` absolute and consistent with the gate's realpath expectations. Absolute-root behavior is byte-identical to prior behavior.

| Finding | Fix | Evidence |
|---------|-----|----------|
| R3-F3-N1 (WARNING, real, live-verified by reliability) — relative scan-root gap: `walkDirs` records raw (relative) dirs when `dir` is passed relative; two-tier exact/case-fold lookup vs absolute `declaredAbs` never fires → existing declared file `unscannable`; without `--force` a misleading lockout refusal, with `--force` the declared skill installs with its content never content-scanned | `skill-scanner.ts`: `await walk(rootAbs)` instead of `await walk(dir)` (R3-F3-N1 comment). Binding test: `skill-scanner.test.ts` `resolves a declared dir through a RELATIVE scan root — no unscannable fallback (R3-F3-N1)` — `path.relative(process.cwd(), tmpDir)` root + declared `skills/Alpha` → declared `pass` reading the real on-disk file, `undeclared: []`, no errors; asserts the invocation is genuinely relative | `src/lib/skill-scanner.ts`, `src/lib/skill-scanner.test.ts` |

**F4 results (real exit codes)**: `pnpm test` — **96 files / 1786 pass / 2 pre-existing skips** (F3 baseline 1785 + 1 new test); `pnpm typecheck` 0; `pnpm lint` 0; pre-commit CI (lint+build) passed. Commit: `977cd28a` (fix + test + ledger F4 section). Ledger R3-F3-N1 → `fixed`.

**Family closed (5 documented instances)**: JD-011 file lowercase (F1) → R1-001 file any-fold (F2) → R1-F2-N1 dir-name fold (F3) → R3-F3-N1 relative-root invocation (F4). Known residual (deliberate trade, documented twice): case-variant TWIN dirs pass membership as declared (inert — consumers construct exact-case paths).
