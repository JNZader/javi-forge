...
- **Linux guard installation/repair** — the `acl` package (provides `getfacl`) is required to install or repair the Claude, Grok Build, and Cursor SkillGuard hooks: the transactional installer proves every controlling directory carries no endangering ACL, and refuses fail-closed when `getfacl` is unresolvable. Install with `apt install acl`, `apk add acl`, or `dnf install acl` (slim container images usually omit it). An already-installed guard keeps working without it; `javi-forge hooks doctor <claude|grok|cursor>` reports installed-state health.
- **`node` on `PATH`** — agent hosts spawn the guard in exec form, so `node` must resolve on the `PATH` the host itself uses, not only inside javi-forge.
