...
3. The installer preflights the asset path and Claude settings and classifies ownership. On fresh installs with absent parents, it may safely create and record restrictive parent directories before same-directory staging; it does not claim impossible zero mutation before staging.
...
`javi-forge hooks doctor claude` is read-only. It reports, separately, the settings entry, installed asset, version/hash, command/args shape, matcher, Node availability, and whether the project is runnable, blocked, inconclusive, missing, outdated, edited-managed, or blocked by foreign content. It evaluates observable `disableAllHooks`, managed hook-source restrictions, strict plugin-only hook customization, and safe mode. Unobservable higher-precedence host policy is `INCONCLUSIVE` with non-zero exit and manual Claude resolved-settings/`/hooks` checks. It also repeats the timeout/spawn fail-open residual instead of returning a blanket "secure" verdict.
...
Settings are parsed and merged structurally, not replaced wholesale. The installer preflights all existing targets, safely creates/revalidates missing parents when necessary, writes through same-directory temporary files, and renames atomically. Any failed preflight or backup leaves target files unmodified and removes only transaction-created directories that remain identity-matched and empty. A settings backup taken for forced managed repair contains the complete prior settings bytes so user content can be restored exactly.
...
- **Node/platforms:** use the package's existing Node `>=22` baseline and exec-form `command`/`args`; bound Windows drive/UNC/extended aliases and Darwin case-insensitive comparisons explicitly; test Bash/PowerShell schemas separately; continuously validate the focused slice on `windows-latest`.
