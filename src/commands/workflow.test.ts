...
import { runWorkflowShow, runWorkflowValidate, runWorkflowList } from './workflow.js'
import type { WorkflowGraph, InitStep } from '../types/index.js'
...
const onStep = (step: InitStep) => { steps.push(step) }
...
describe('runWorkflowShow', () => {
  it('shows a built-in template', async () => {
    mockedLoadBuiltinTemplate.mockResolvedValue({ content: 'digraph { A -> B }', format: 'dot' })
    mockedParseDot.mockReturnValue(sampleGraph)

    const result = await runWorkflowShow('/test', onStep, { template: 'ci-pipeline' })
  ...
  it('shows a file by path', async () => {
    ...
    const result = await runWorkflowShow('/test', onStep, { target: '/test/pipeline.dot' })
    expect(result).toBe('[Lint] -> [Test]')
  })

  it('auto-discovers workflows', async () => {
    mockedDiscoverWorkflows.mockResolvedValue([
      { name: 'deploy', path: '/test/.javi-forge/workflows/deploy.dot', format: 'dot' },
    ])
    mockedFs.readFile.mockResolvedValue('digraph { A -> B }' as never)
    mockedParseDot.mockReturnValue(sampleGraph)
  ...
  it('returns null when template not found', async () => {
    ...
    const result = await runWorkflowShow('/test', onStep, { template: 'nonexistent' })
...
describe('runWorkflowValidate', () => {
  it('validates and renders with results', async () => {
    ...
    const result = await runWorkflowValidate('/test', onStep, { template: 'ci-pipeline' })
    expect(result).toBe('[Lint] -> [Test]')
    expect(mockedValidateWorkflow).toHaveBeenCalledWith(sampleGraph, '/test')
    expect(mockedRenderAscii).toHaveBeenCalledWith(sampleGraph, expect.any(Array))
  })
})
