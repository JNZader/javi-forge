...
export async function generateProjectSkillsJson(
...
): Promise<{ success: boolean; path?: string; skillCount: number; pluginCount: number; error?: string }> {
  const { dryRun = false, registryName } = options
  const pluginsDir = path.join(projectDir, '.javi-forge', 'plugins')

  if (!await fs.pathExists(pluginsDir)) {
    return { success: false, skillCount: 0, pluginCount: 0, error: 'no plugins directory found' }
  }
  ...
  if (plugins.length === 0) {
    return { success: false, skillCount: 0, pluginCount: 0, error: 'no installed plugins found' }
  }
