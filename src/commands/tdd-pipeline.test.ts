...
import { generateTddPipelineHook, installTddPipelineHook } from './tdd-pipeline.js'
...
describe('installTddPipelineHook', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'javi-forge-tdd-pipeline-'))
  })
  ...
  it('backs up existing pre-push hook', async () => {
    ...
    await fs.writeFile(path.join(tmpDir, '.git', 'hooks', 'pre-push'), existingHook)
    await fs.writeJson(path.join(tmpDir, 'package.json'), {
      scripts: { test: 'jest' },
    })
    await fs.writeFile(path.join(tmpDir, 'pnpm-lock.yaml'), '')
    ...
    const newContent = await fs.readFile(path.join(tmpDir, '.git', 'hooks', 'pre-push'), 'utf-8')
