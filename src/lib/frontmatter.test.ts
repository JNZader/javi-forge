...
describe("validateFrontmatter", () => {
	...
	it("returns no errors for valid skill with Trigger:", () => {
		const fm = {
			name: "my-skill",
			description: "A valid skill. Trigger: when testing",
		};
	...
	it("returns error for name too short (1 char)", () => {
		...
		const errors = validateFrontmatter(fm, "agent");
		expect(
			errors.some((e) => e.field === "name" && e.message.includes("2-60")),
		).toBe(true);
	});
	...
	it("returns error for uppercase name", () => {
		...
		expect(
			errors.some(
				(e) => e.field === "name" && e.message.includes("kebab-case"),
			),
		).toBe(true);
	});
	...
	it("returns error for description too short (9 chars)", () => {
		...
		const errors = validateFrontmatter(fm, "agent");
		expect(
			errors.some((e) => e.field === "description" && e.message.includes("10")),
		).toBe(true);
	});
	...
	it("returns error for skill without Trigger:", () => {
		...
		expect(
			errors.some(
				(e) => e.field === "description" && e.message.includes("Trigger:"),
			),
		).toBe(true);
	});
