...
### Decision 5: Prove a private parent chain, then stage the complete two-target transaction

**Choice**: Classify settings and asset independently, then compute one plan. Before mutation, the manager MUST establish the parent-chain contract below. It keeps a no-follow handle for every existing directory from the filesystem root through each target parent, records identity from the handle, and reopens/revalidates each path against the held identity immediately before and after every pathname mutation. A fresh install may then create missing `.claude` and `.claude/hooks` parents one segment at a time; these directory creations are the transaction's first mutations and are explicitly recorded. Each new directory is created exclusively, opened no-follow, permission/ACL-verified, and added to the held chain before use. Only after those parents exist does the manager create same-directory backups and staged files. Before either target-file rename, it validates package assets/manifest, parses settings, validates every target/parent/backup state, captures prior bytes and permissions, creates all required exclusive backups, and writes/fsyncs all temporary files. It commits asset first and settings second using same-directory rename. If a later commit fails and the written target still matches this transaction's bytes, it restores every prior target from a prepared same-directory rollback temp (or removes a target that was previously absent), then removes only transaction-created directories that are still identity-matched and empty.
...
**Rationale**: Node `fs` has no portable `openat(2)`/`renameat(2)` or Windows handle-relative rename API. Holding a directory `FileHandle`, calling `stat`, and performing `lstat` before/after a pathname operation does **not** alone stop an attacker from swapping the parent out and back between observations. Therefore this design does not claim portable race-free dirfd-relative replacement. Its security boundary additionally refuses every chain on which a principal outside the trusted set can rename/create/delete a traversed entry; the held identities detect accidental/concurrent changes, while the access check excludes the double-swap attacker from the supported threat model. The trusted set is the current effective user plus root on POSIX, and the current user SID plus `SYSTEM` and `BUILTIN\\Administrators` on Windows; compromise of those principals, kernel/filesystem faults, and malicious same-user processes are residuals. Same-directory staging is impossible before an absent parent exists, so the design also does not promise zero mutation before staging on a fresh install. Asset-first avoids a normally observable settings entry pointing at a missing file. Atomic rename prevents torn individual files. Cross-file atomicity and crash-atomic parent cleanup are unavailable, so guarded rollback and component-level doctor status are required.

The implementable parent-chain gate is platform-specific and fail-closed:

- **Linux/POSIX implementation**: require `O_DIRECTORY` and `O_NOFOLLOW`; open every existing directory and compare `fstat`/reopened `stat` `dev`+`ino`. Every directory that controls the next path component, including both final target parents, must be owned by the effective uid or root and have no group/other write bits. The Linux ACL adapter executes `getfacl --absolute-names --numeric --omit-header -- <path>` with a bounded timeout and locale fixed to `C`; any named user/group entry, `mask::`, `default:*` entry, parse error, unavailable executable, unsupported filesystem result, or changed output during preflight is inconclusive and refuses. Other POSIX hosts are unsupported for mutation unless they implement an equally specified adapter. This deliberately refuses shared/group-writable trees rather than relying on sticky-bit subtleties; the remediation is to move/use a private `0700`-ancestor project tree.
- **macOS implementation**: open directories with `O_DIRECTORY|O_NOFOLLOW` and compare `dev`+`ino` as above; require effective-user/root ownership and no group/other write bits. Execute `/bin/ls -lde -- <path>` with locale `C` and a bounded timeout; any numbered ACL entry, inherited ACL entry, parse/tool error, or output change is inconclusive and refuses. The implementation does not use `chmod -N` and never strips ACLs. APFS/HFS path identity is supported only where the opened-handle identity remains stable.
- **Windows implementation**: refuse UNC/network paths, reparse points, and volumes other than local NTFS/ReFS with stable file IDs. The packaged hashed helper opens every directory using `CreateFileW(OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS|FILE_FLAG_OPEN_REPARSE_POINT)`, keeps those handles alive for the transaction, and compares volume serial plus `FILE_ID_INFO` on handle and reopened path around every mutation. It parses the DACL and refuses any allow ACE granting create/write/delete-child/delete/write-DAC/write-owner rights to a SID outside the current user, `SYSTEM`, or `BUILTIN\\Administrators`; unknown/conditional/object ACEs with mutation rights, unavailable token/DACL resolution, or an unprotected ambiguous descriptor refuse. File creation/rename/cleanup remains in that helper while handles are held. The helper cannot make two target renames one transaction.
- **All platforms**: if handle opening, identity, owner, ACL/DACL, local-filesystem semantics, or exclusive creation cannot be proven exactly, install/repair refuses before target-file mutation with reason `unsafe-parent-chain` and names the first path/capability. It never falls back to pathname checks alone. Directory cleanup uses the same gate and stops for manual recovery if proof is lost.
...
Examples are `settings.json.javi-forge.bak.20260813T143015042Z.a1b2c3d4` and `javi-forge-skillguard-pre-tool-use.mjs.javi-forge.bak.20260813T143015042Z.a1b2c3d4`. The clock and nonce source are injectable in tests. POSIX creation uses same-directory `open` with `O_CREAT|O_EXCL|O_WRONLY|O_NOFOLLOW` and mode `0o600` at creation time, with a bounded eight-candidate nonce retry; bytes are not written before restrictive creation succeeds. Before any replacement or forced backup, the platform ACL adapter must prove that the source has **no extended ACL** and that the parent has no default/inheritable ACL. Linux uses the exact `getfacl` contract in Decision 5; macOS uses `/bin/ls -lde`. Node has no portable POSIX ACL API, so tool absence, unsupported output, a named/mask/default/inherited ACL, or a changed ACL result causes `unsupported-posix-acl` refusal before content is written. The manager never strips an ACL or silently substitutes mode bits. Settings backup contains the complete original bytes; asset backup contains the complete original bytes. It writes and fsyncs while the file remains `0o600`, applies the exact source mode without adding a permission bit, verifies mode and ACL absence again, fsyncs metadata, and closes. Consequently a backup/replacement is equal to or narrower than the source; an ACL-bearing source must be handled manually and is never replaced by this release.

On Windows, the manager uses the packaged, hashed `assets/claude-hooks/javi-forge-windows-secure-object.ps1` helper for both the parent-handle contract and DACL-safe creation. Its .NET interop calls `CreateFileW(CREATE_NEW)` or `CreateDirectoryW` with an explicit `SECURITY_ATTRIBUTES` descriptor. For files, the helper receives framed bytes on stdin, atomically creates the temp/backup with the source DACL (replacement/backup) or a protected DACL granting only the current user plus required SYSTEM/Administrators principals (new file), writes through the still-open handle, calls `FlushFileBuffers`, and closes. For missing parents, it creates each directory with the protected DACL already attached. No path exists where a file or directory is first created with an inherited broad ACL and tightened later. The manager verifies the helper hash before use and recaptures/verifies each resulting DACL before staging/rename. If helper integrity, parent identity, local-volume support, DACL capture, creation, or verification is unavailable/ambiguous, or if the resulting DACL would broaden access relative to the source, the operation refuses before target-file mutation and removes only identity-matched empty parents it created. Any backup failure aborts before target mutation.

### Atomic write, fsync, rename, and rollback

For every file to replace:

1. Establish Decision 5's held-handle, identity, private-parent, local-filesystem, and ACL/DACL gate for the complete chain. A pair of pathname `lstat` checks is never accepted as proof. Create each missing `.claude`/`.claude/hooks` segment exclusively with mode `0o700` and no ACL on POSIX or the verified Windows protected-directory DACL, then open/verify/hold it before the next operation. Never use blind recursive mkdir.
2. Through the still-valid gate, record target bytes, SHA-256, identity, exact POSIX mode plus proven ACL absence, or exact Windows DACL. Refuse any extended/inconclusive POSIX ACL state.
...
4. On POSIX, write and `handle.sync()` while mode remains `0o600`, apply the exact prior mode for replacements, verify mode and ACL absence, sync metadata, and close; new files remain `0o600` with verified ACL absence. On Windows, the helper writes and flushes through the securely created handle, after which it verifies the resulting DACL. Refuse if source access preservation/no-broadening cannot be proven.
5. Immediately before rename, revalidate every held ancestor identity and permission/ACL result, then no-follow open/re-hash the destination; if anything differs from preflight, abort as a concurrent modification. This check detects drift but is not represented as protection against swap-out/swap-back without the private-parent gate.
6. Rename temp to destination through the gated platform adapter, then revalidate the chain and committed target. Sync the parent directory on POSIX. On Windows, file flush plus rename is the documented durability boundary; only known unsupported directory-sync errors are ignored.
7. Commit asset, then settings. After each rename, record the exact committed hash.
8. If a later operation fails, roll back in reverse order only while the full parent gate remains valid and the current path still matches the transaction's committed hash. Restore prior bytes/mode/no-ACL or DACL through another restrictively created, verified, fsynced same-directory temp+rename, or unlink a newly created target and sync the directory. Remove each transaction-created directory in reverse order only while its held identity still matches and it is empty. Lost proof, a concurrent post-write change, or a non-empty/newly replaced directory stops automatic cleanup and produces manual recovery guidance.
9. Remove owned leftover temps only through the same gate. A hard process/host crash may leave a temp or one committed component; doctor reports it as partial. It never removes an ambiguous temp-like path.

The MJS is launched by Node and is created as a normal non-executable file; no shebang or executable bit is required. Existing regular-file modes and Windows DACLs are preserved exactly or the operation refuses. POSIX files with any extended ACL are refused rather than mode-only replaced. The honest limits are that Node cannot provide portable dirfd-relative rename, malicious same-user/trusted-administrator processes are outside the parent-swap boundary, and a hard crash can leave identity-verified empty parents, temps, or one committed component; no design can make parent creation plus two file renames one filesystem transaction. Doctor reports those remnants/partial state, and normal rollback never removes an unrecognized or non-empty directory.
...
- `javi-forge hooks doctor claude`: apply the authoritative matrix below. Output is component-level, prints exactly one of `RUNNABLE`, `BLOCKED`, or `INCONCLUSIVE` prominently, and always states the five-tool coverage plus host residual; only `RUNNABLE` sets `healthy: true`.
...
Doctor resolves effective hook execution separately from file ownership. Every source in the supported inventory receives `clear`, `blocking`, or `unknown`: project `.claude/settings.json`, local `.claude/settings.local.json`, user `~/.claude/settings.json`, documented managed settings files/drop-ins for the host OS, supported server/MDM representations or resolved-settings probe, `disableAllHooks`, `allowManagedHooksOnly`, strict plugin-only customization covering hooks, and safe-mode environment/current-launch state. Missing unreadable higher-precedence files, unsupported/malformed policy, inability to prove source absence, and launch policy not exposed by a supported probe are `unknown`, not `clear`. A flag or remote policy applied only to a later launch remains a disclosed temporal residual; if it is part of the launch being diagnosed and is unobservable, it is `unknown`.

| Ordered condition | Overall status | `healthy` | Exit | Deterministic message/remediation |
|---|---|---:|---:|---|
| Any known blocker: component not managed-current, command/matcher mismatch, Node `<22`/unavailable, effective `disableAllHooks`, managed-only/plugin-only exclusion, or observed safe mode | `BLOCKED` | false | `1` | Print sorted stable blocker IDs and each ID's fixed remediation (`install`, eligible `repair [--force]`, remove the named disabling policy, exit safe mode, or install Node 22), then `rerun javi-forge hooks doctor claude`. Known blockers dominate simultaneous unknowns because execution is already proven blocked; unknown IDs are still listed as secondary evidence. |
| No known blocker, but any relevant inventory source is `unknown` | `INCONCLUSIVE` | false | `2` | Print sorted stable unknown-source IDs, `installed bytes do not prove execution`, then exactly `Run Claude's claude doctor or /status and /hooks outside safe mode; resolve the named policy source; rerun javi-forge hooks doctor claude.` |
| Both components current; shape/matcher exact; Node `>=22`; every relevant inventory source explicitly `clear`; no blocker or unknown | `RUNNABLE` | true | `0` | Print five-tool coverage and `Host residual: spawn, start, termination, and timeout failures may continue through Claude permission flow.` No secure/fail-closed badge. |

The implementation evaluates rows top-to-bottom and sorts IDs by the committed inventory order, so status, exit, and remediation do not depend on filesystem enumeration order. Failure to detect a policy source is never promoted to `clear`; therefore `RUNNABLE` can be produced only on a platform/context where supported readable representations or a supported resolved-settings probe prove every relevant source clear.
...
| Transaction | absent-parent creation, held directory identities, private-parent refusal, swap and swap-back attempts, backup collisions/failure, restrictive temp mode/ACL before write, temp open/write/fsync/rename failure, second-target failure, successful rollback, empty-parent cleanup, interruption partial state, permissions | Adapter unit tests always run and inject handle/identity/ACL/DACL/tool failures. Real POSIX tests require a private ancestor and assert group/world-writable refusal. Linux ACL tests use `setfacl`/`getfacl`; macOS ACL tests use `chmod +a` and `/bin/ls -lde`; creation-of-ACL fixtures skip only when the test host lacks that capability, while mocked unavailable/inconclusive-tool refusal tests never skip. Windows CI tests reparse/network/unstable-ID and untrusted-write ACE refusal. Assert exact pre-operation bytes/modes/DACLs and no target mutation on refusal. |
| Init/CLI | shared installer, Minimal opt-in trigger matrix, dry-run, incomplete reporting, force eligibility, complete doctor matrix/read-only behavior, exit codes/help | Unit tests cover every blocker and every unknown source, blocker+unknown precedence, all-clear RUNNABLE, stable ID ordering, exact exit code, and exact remediation text. Selected integration fixtures use real filesystem. Existing Git `hooks run` tests remain green. |
