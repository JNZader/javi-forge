...
export default function AnalyzeUI({ dryRun }: Props) {
	...
	useEffect(() => {
		runAnalyze(process.cwd(), dryRun, (step) =>
			setSteps((prev) => {
			...
			{!finished && steps.length === 0 && (
				<Box marginLeft={2}>
					<Text color={theme.warning}>
						<Spinner type="dots" /> Checking for repoforge...
					</Text>
				</Box>
			)}
			...
			{finished && (
				<Box marginTop={1} flexDirection="column">
					<Text bold color={errorCount > 0 ? theme.warning : theme.success}>
						...
						{elapsed && (
							<Text color={theme.muted}> Completed in {elapsed}</Text>
						)}
					</Text>
					{doneCount > 0 && (
						<Text color={theme.success}>
							{" "}
							{"\u2713"} {doneCount} checks passed
						</Text>
					)}
					{errorCount > 0 && (
						<Text color={theme.error}>
