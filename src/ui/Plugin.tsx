  ...
  useEffect(() => {
    const run = async () => {
      try {
        switch (action) {
          case 'add':
            if (!target) { onStep({ id: 'err', label: 'Error', status: 'error', detail: 'source required: javi-forge plugin add <org/repo>' }); break }
            await runPluginAdd(target, dryRun, onStep)
            break
          case 'remove':
            if (!target) { onStep({ id: 'err', label: 'Error', status: 'error', detail: 'name required: javi-forge plugin remove <name>' }); break }
