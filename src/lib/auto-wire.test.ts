...
describe("buildAutoWireSection", () => {
	it("produces section with start/end markers", () => {
		const plugins = [
			makePlugin({
				name: "my-plugin",
				version: "2.0.0",
				description: "Does cool things for devs",
			}),
		];
	...
	it("includes plugin name, version, and description", () => {
		const plugins = [
			makePlugin({
				...
				version: "3.1.0",
				description: "Scans for vulnerabilities in code",
			}),
		];
		const result = buildAutoWireSection(plugins);
	...
	it("includes skill loader lines", () => {
		const plugins = [
			makePlugin({
				name: "react-tools",
				skills: ["react-pro", "hooks-linter"],
			}),
		];
		const result = buildAutoWireSection(plugins);

		expect(result).toContain(
			"~/.claude/plugins/react-tools/skills/react-pro/SKILL.md",
		);
		expect(result).toContain(
			"~/.claude/plugins/react-tools/skills/hooks-linter/SKILL.md",
		);
	});

	it("includes command entries", () => {
		const plugins = [
			makePlugin({ name: "my-tools", commands: ["lint", "format"] }),
		];
		const result = buildAutoWireSection(plugins);
...
describe("autoWirePlugins", () => {
	it("wires skills into CLAUDE.md", async () => {
		...
		const plugins = [
			makePlugin({ name: "react-tools", skills: ["react-pro"] }),
		];
		...
		expect(
			result.wired.some(
				(w) => w.capability === "skill" && w.value === "react-pro",
			),
		).toBe(true);
	...
	it("wires hooks into settings.json", async () => {
		...
		const plugins = [
			makePlugin({ name: "hooks-plugin", hooks: ["pre-commit-check"] }),
		];
		const result = await autoWirePlugins("/fake/project", plugins);

		expect(
			result.wired.some(
				(w) => w.capability === "hook" && w.value === "pre-commit-check",
			),
		).toBe(true);
	...
	it("wires commands and agents into CLAUDE.md", async () => {
		...
		expect(
			result.wired.some(
				(w) => w.capability === "skill" && w.value === "skill-x",
			),
		).toBe(true);
		expect(
			result.wired.some(
				(w) => w.capability === "command" && w.value === "cmd-y",
			),
		).toBe(true);
		expect(
			result.wired.some(
				(w) => w.capability === "agent" && w.value === "agent-z",
			),
		).toBe(true);
	});
	...
	it("skips already-wired hooks in settings.json (idempotent)", async () => {
		...
		const plugins = [
			makePlugin({ name: "hook-plugin", hooks: ["existing-hook"] }),
		];
		const result = await autoWirePlugins("/fake/project", plugins);
