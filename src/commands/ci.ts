...
export async function detectCIStack(projectDir: string): Promise<CIStackInfo> {
  ...
  // Node
    ...
    stackType = 'node'
    if (await fs.pathExists(path.join(projectDir, 'pnpm-lock.yaml'))) buildTool = 'pnpm'
    else if (await fs.pathExists(path.join(projectDir, 'yarn.lock'))) buildTool = 'yarn'
    else buildTool = 'npm'
  }
  // Go
  ...
  // Python
    ...
    else if (await fs.pathExists(path.join(projectDir, 'poetry.lock'))) buildTool = 'poetry'
    else buildTool = 'pip'
  }
...
async function buildCICommands(
  ...
  buildTool: string,
  projectDir: string
): Promise<{ lintCmd: string | null; compileCmd: string | null; testCmd: string | null }> {
  switch (stack) {
    case 'java-gradle':
      return {
...
function report(onStep: CIStepCallback, id: string, label: string, status: CIStepStatus, detail?: string) {
  onStep({ id, label, status, detail })
}

export async function runCI(options: CIOptions, onStep: CIStepCallback): Promise<void> {
  const {
...
export async function installCIHooks(projectDir: string): Promise<{ installed: string[]; errors: string[] }> {
