...
export async function autoWirePlugins(
  ...
  try {
    const claudeResult = await wireClaudeMd(projectDir, desiredSkills, installedPlugins, dryRun)
    wired.push(...claudeResult.wired)
    unwired.push(...claudeResult.unwired)
  } catch (e: unknown) {
    errors.push(`CLAUDE.md wiring failed: ${e instanceof Error ? e.message : String(e)}`)
  }

  // ── Wire hooks into .claude/settings.json ────────────────────────────
  try {
    const hooksResult = await wireSettingsJson(projectDir, desiredHooks, installedPlugins, dryRun)
    wired.push(...hooksResult.wired)
    unwired.push(...hooksResult.unwired)
  } catch (e: unknown) {
    errors.push(`settings.json wiring failed: ${e instanceof Error ? e.message : String(e)}`)
  }
 ...
 */
export function buildAutoWireSection(
  ...
  for (const plugin of plugins) {
    ...
    if (skills.length > 0) {
      for (const skill of skills) {
        lines.push(`- Load \`~/.claude/plugins/${manifest.name}/skills/${skill}/SKILL.md\` when working with ${skill}`)
      }
      lines.push('')
    }
...
async function wireClaudeMd(
  ...
  for (const plugin of pluginsWithCapabilities) {
    for (const cmd of plugin.manifest.commands ?? []) {
      wired.push({ plugin: plugin.manifest.name, target: 'claude-md', capability: 'command', value: cmd })
    }
    for (const agent of plugin.manifest.agents ?? []) {
      wired.push({ plugin: plugin.manifest.name, target: 'claude-md', capability: 'agent', value: agent })
    }
  }
...
async function wireSettingsJson(
  ...
  const cleanedHooks = pluginHooks.filter(h => {
    ...
    if (!belongsToActive) {
      unwired.push({ plugin: 'unknown', target: 'settings-json', capability: 'hook', value: h })
      return false
    }
    return true
  })
