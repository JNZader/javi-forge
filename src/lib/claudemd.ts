...
const PLUGIN_INSTRUCTIONS: Record<string, string> = {
	"react-19":
		"Check `~/.claude/plugins/` for merge-checks and mermaid diagram support",
	"nextjs-15":
		"Check `~/.claude/plugins/` for merge-checks and mermaid diagram support",
...
export function buildClaudeMd(
	...
	if (entry.skills.length > 0) {
		...
		for (const skill of entry.skills) {
			lines.push(
				`- Load \`~/.claude/skills/${skill}/SKILL.md\` before writing code`,
			);
		}
		lines.push("");
	}
	...
	lines.push(
		"- Scaffolded with javi-forge — see `.javi-forge/manifest.json` for config",
	);
	lines.push("");
