...
- This is GUARDED, not documented-and-hoped: a test asserts that for every hook, `manifest.historical[]` CONTAINS the sha256 of the previous released asset body (read via `git show <prev-tag>:assets/hooks/<hook>`, skipped only when git history is unavailable). A body change that forgets the append fails CI instead of silently bricking `ci init` for every installed repo — exactly the R2 failure this whole design exists to prevent.
