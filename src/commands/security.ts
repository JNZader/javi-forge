...
export type SecurityStepStatus =
	| "pending"
	| "running"
	| "done"
	| "error"
...
export function parseNpmAudit(raw: string): SecurityFinding[] {
	...
	try {
		...
		for (const [pkgName, info] of Object.entries(vulns)) {
			...
			const directVias = (v.via ?? []).filter(
				(x): x is { title?: string; url?: string; source?: number } =>
					typeof x === "object",
			);
			...
			for (const vuln of dep.vulns ?? []) {
				findings.push({
					...
					severity: normalizeSeverity(
						vuln.fix_versions?.length ? "high" : "moderate",
					),
					package: dep.name,
	...
	try {
		...
		for (const v of vulns) {
			...
			findings.push({
				...
				title:
					advisory.title ?? `Vulnerability in ${v.package?.name ?? "unknown"}`,
				url: advisory.url,
			});
		}
	} catch {
...
export function detectRegressions(
	...
	let regressions = current.filter(
		(f) => !baselineKeySet.has(makeFindingKey(f)),
	);
	const resolved = baseline.findings.filter(
		(f) => !currentKeySet.has(makeFindingKey(f)),
	);
...
export async function runSecurity(
	...
	if (!auditCmd) {
		...
		throw new Error(
			`Unsupported stack for security audit: ${stackInfo.stackType}`,
		);
	}
	...
	switch (mode) {
		...
		case "update": {
			report(
				onStep,
				"save",
				mode === "update" ? "Updating baseline" : "Creating baseline",
				"running",
			);
			...
			report(
				onStep,
				"save",
				`Baseline saved with ${findings.length} finding(s)`,
				"done",
		...
		case "check": {
			...
			if (!existing) {
				...
				throw new Error(
					"No security baseline found. Run `javi-forge security baseline` first.",
				);
			}
			...
			if (sevBreakdown) {
				report(
					onStep,
					"summary",
					`Current findings: ${summary.total} (${sevBreakdown})`,
					"done",
				);
			}
			...
			if (result.filteredRegressions.length === 0) {
				...
				const belowThreshold =
					result.regressions.length > result.filteredRegressions.length
						? ` (${result.regressions.length - result.filteredRegressions.length} below threshold)`
						: "";
				report(
					onStep,
					"check",
					`No actionable regressions${resolvedMsg}${belowThreshold}`,
					"done",
				);
			} else {
				...
				report(
					onStep,
					"check",
					`${result.filteredRegressions.length} regression(s) found`,
					"error",
					details,
				);
			}
		...
		case "allowlist": {
			...
			report(
				onStep,
				"allowlist",
				`Allowlist updated: ${existingAllowlist.size} finding(s) allowed`,
				"done",
				baselinePath(projectDir),
			);
			return null;
