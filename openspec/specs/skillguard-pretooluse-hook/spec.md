...
### Requirement: Guard lifecycle is supported only on Linux and Windows

The package and every public install, repair, init, and doctor entrypoint MUST support Linux and Windows only. Every other host MUST return the same generic `unsupported-platform` outcome before filesystem, settings, ACL/DACL, subprocess, or transaction work. No unsupported-host migration, cleanup, compatibility, or grace path is part of this lifecycle.

#### Scenario: Unsupported host refuses before guard work

- GIVEN a guard lifecycle entrypoint runs on a host other than Linux or Windows
- WHEN platform eligibility is evaluated
- THEN it returns generic `unsupported-platform` guidance naming Linux and Windows
- AND it performs no guard probe, settings inspection, ACL/DACL operation, or mutation
...
- GIVEN a Linux managed source has an extended ACL, its parent has a default/inheritable ACL, or the required ACL inspector cannot prove ACL absence
