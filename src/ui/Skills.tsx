			...
			{result && (
				<Box flexDirection="column">
					...
					<Box flexDirection="column" marginBottom={1}>
						<Text
							bold
							color={result.budget.overBudget ? theme.warning : theme.success}
						>
							{"  "}Context Budget
						</Text>
						<Box marginLeft={2}>
							...
							{result.budget.optimizations.map((opt, i) => (
								<Box key={`opt-${i}`} flexDirection="column" marginLeft={4}>
									<Text color={opt.meetsbudget ? theme.success : theme.warning}>
										{opt.meetsbudget ? "\u2713" : "~"} Disable [
										{opt.disableSkills.join(", ")}] {"\u2192"} save ~
										{opt.tokensSaved} tokens ({opt.remainingTokens} remaining)
									</Text>
								</Box>
							))}
						</Box>
