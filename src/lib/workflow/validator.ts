 ...
 */
const BUILTIN_CHECKS: Record<string, (projectDir: string) => Promise<{ pass: boolean; detail: string }>> = {
  'has-linter': async (dir) => {
    const eslintConfig = ['.eslintrc', '.eslintrc.js', '.eslintrc.json', '.eslintrc.yml', 'eslint.config.js', 'eslint.config.mjs', 'eslint.config.ts']
    ...
    if (await fs.pathExists(pkgPath)) {
      try {
        ...
        const deps = { ...(pkg['devDependencies'] as Record<string, string> | undefined), ...(pkg['dependencies'] as Record<string, string> | undefined) }
  ...
  'has-tests': async (dir) => {
    const testPatterns = [
      ...
      'src/**/*_test.go', '*_test.go',
      'tests/test_*.py', 'test_*.py',
    ]
    for (const pattern of testPatterns) {
      const target = path.join(dir, pattern.split('*')[0] ?? '')
      if (await fs.pathExists(target)) {
        return { pass: true, detail: `Found test path: ${pattern.split('*')[0]}` }
      }
    }
  ...
  'has-security': async (dir) => {
    const secPaths = ['.javi-forge/security-baseline.json', 'semgrep.yml', '.semgrep.yml', '.snyk']
...
export async function validateWorkflow(
  ...
  for (const node of graph.nodes) {
    if (!node.check) {
      results.push({ node: node.id, status: 'skip', detail: 'No check defined' })
      continue
    }

    const checkFn = BUILTIN_CHECKS[node.check]
    if (!checkFn) {
      results.push({ node: node.id, status: 'skip', detail: `Unknown check: ${node.check}` })
      continue
    }
