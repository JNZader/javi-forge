...
const PROFILES: ProfileEntry[] = (
	Object.entries(HOOK_PROFILES) as [
		HookProfile,
		(typeof HOOK_PROFILES)[HookProfile],
	][]
).map(([id, meta]) => ({
	id,
	label: meta.label,
...
export default function HookProfileSelector({
	onConfirm,
	presetProfile,
}: Props) {
	...
	return (
		<Box flexDirection="column">
			...
			<Box marginTop={1} gap={2}>
				<Text color={theme.primary}>{PROFILES[cursor].label}</Text>
				<Text color={theme.muted} dimColor>
