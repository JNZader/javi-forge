...
describe("renderTemplate", () => {
	...
	it("replaces multiple different placeholders", async () => {
		mockedFs.readFile.mockResolvedValue(
			"__GREETING__ __NAME__, age __AGE__" as never,
		);
	...
	it("generates for a single stack", async () => {
		mockedFs.readFile.mockImplementation(async (filePath: unknown) => {
			...
			if (p.includes("github-actions.yml"))
				return "  - package-ecosystem: github-actions\n" as never;
...
describe("generateDeployWorkflow", () => {
	...
	it("returns null when no template mapping exists", async () => {
		const result = await generateDeployWorkflow(
			"bitbucket" as CIProvider,
			"app",
		);
		expect(result).toBeNull();
	});
	...
	it("defaults service name in replacement", async () => {
		mockedFs.pathExists.mockResolvedValue(true as never);
		mockedFs.readFile.mockResolvedValue(
			'docker rollout "__SERVICE_NAME__"' as never,
		);
