...
> **Importante:** este primer slice solo publica la variante shell para Linux/Mac y depende de `lib/common.sh`. Copiar siempre ambos directorios.
...
└── docker/
    └── *.Dockerfile  # Se generan automaticamente

lib/                  # Dependencia requerida (copiar junto con .ci-local/)
└── common.sh         # Funciones compartidas (Bash)
```

La compatibilidad Windows/PowerShell queda fuera del alcance de este primer slice de WI-023.
